...
namespace Vulyk.Services
{
    public class UserService : IUserService
    {
        ...
        /// </list>
        /// </returns>
        public async Task<Result<UserProfileEditDto>> GetUserProfileAsync(string id)
        {
            var foundUser = await _userManager.FindByIdAsync(id);
            if (foundUser == null)
            {
                _logger.LogWarning("Failed to get user profile: User with UserId={id} not found", id);
                return Result.Fail(new Error("User not found").WithMetadata("Code", "UserNotFound"));
            }
            ...
            UserProfileEditDto? user = await _context.ApplicationUser
                ...
                .Select(u => new UserProfileEditDto { Email = u.Email!, FullName = u.FullName!, PhoneNumber = u.PhoneNumber, IsPasswordExist = isPasswordExist })
                .FirstOrDefaultAsync();
            if (user == null)
            {
                _logger.LogWarning("Failed to get user profile with UserId={id}", id);
                return Result.Fail(new Error("User not found").WithMetadata("Code", "UserNotFound"));
            }
        ...
        /// </list>
        /// </returns>
        public async Task<Result> EditFullNameAsync(string id, string fullName)
        {
            var foundUser = await _userManager.FindByIdAsync(id);
            if (foundUser == null)
            {
                _logger.LogWarning("Failed to edit user FullName: User with UserId={id} not found", id);
                return Result.Fail(new Error("User not found").WithMetadata("Code", "UserNotFound"));
            }
